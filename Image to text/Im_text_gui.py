...
#__________________________________________________ tab-1 Design ______________________________________________________
        ...
        self.path = ""
        self.path_entry = Entry(self.top, font='consolas 17 ', width=38, textvariable=self.entry_val, bd=2,
                                bg="#ececec")
        self.path_entry.place(x=27, y=158)

        self.chooseBtn = Button(self.top, text="choose", font='consolas 14 bold', width=7, bd=3, command=self.get_file)
        self.chooseBtn.place(x=490, y=155)

        self.submit_btn = Button(self.top, text='Submit', font='consolas 17 bold', fg="#fff", bg="#36a2bb", width=12,
                                 bd=3,command = self.submit_fun)
        self.submit_btn.place(x=615, y=153)

        self.clear_button = Button(self.top, text='Clear', font='Consolas 17 bold', bg='#b90000', fg='#FFF',
                                  width=12,command=lambda: self.clear(0))
        self.clear_button.place(x=27, y=215)

        self.save_btn = Button(self.top,text="Save", font='Consolas 17 bold', bg='#441f11', fg='#FFF',
                                  width=12,command=lambda: self.save_fun(0))
        self.save_btn.place(x=617,y=215)

        scroll = Scrollbar(self.bottom)
        scroll.pack(side=RIGHT, fill=Y)
        self.text = Text(self.bottom, font='Consolas 25 ', wrap=WORD, padx=10, pady=10, bd=2,
                         yscrollcommand=scroll.set,fg="#FFF", bg="#000")
        self.text.pack()
        scroll.config(command=self.text.yview)

    # _______________________________________________ tab-2 Design ____________________________________________________
        ...
        label21.place(x=29, y=120)

        self.entry_val1 = StringVar()
        self.path1 = ""
        self.path_entry1 = Entry(self.top1, font='consolas 17 ', width=38, textvariable=self.entry_val1, bd=2,
                                bg="#ececec")
        self.path_entry1.place(x=27, y=158)

        self.chooseBtn1 = Button(self.top1, text="choose", font='consolas 14 bold', width=7, bd=3, command=self.get_file1)
        self.chooseBtn1.place(x=490, y=155)

        self.submit_btn1 = Button(self.top1, text='Submit', font='consolas 17 bold', fg="#fff", bg="#36a2bb", width=12,
                                 bd=3, command=self.submit_fun1)
        self.submit_btn1.place(x=615, y=153)

        self.clear_button1 = Button(self.top1, text='clear all', font='Consolas 17 bold', bg='#b90000', fg='#FFF',
                                   width=12,command=lambda: self.clear(1))
        self.clear_button1.place(x=27, y=215)

        self.show = Button(self.top1, text='show image', font='Consolas 18 bold', bg='#32a61a', fg='#FFF',
                                    width=11, command=self.show_img)
        self.show.place(x=317, y=215)

        self.save_btn1 = Button(self.top1, text="Save", font='Consolas 17 bold', bg='#441f11', fg='#FFF',
                               width=12, command=lambda: self.save_fun(1))
        self.save_btn1.place(x=617, y=215)

        scroll1 = Scrollbar(self.bottom1)
        scroll1.pack(side=RIGHT, fill=Y)
        self.text1 = Text(self.bottom1, font='Consolas 20 ', wrap=WORD, padx=10, pady=10, bd=2,
                         yscrollcommand=scroll1.set, fg="#FFF", bg="#000")
        self.text1.pack()
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Functions @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def show_img(self):
        if self.path1!='':
            ...
            messagebox.showerror("Error", "You haven't elect any image file")
