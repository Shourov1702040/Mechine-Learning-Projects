...
class Pepper_plant_frame(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry('800x650+1000+200')
        self.title("Detect Pepper plant disease")
        ...
        ####################### top frame desigh  ###########################################
        # self.top_image = PhotoImage(file=r'icon_image/corn_plant2.png')
        img_jpg = Image.open(r"icon_image/peeper_plant.jpg")
        ...
        self.heading = Label(self.top, text='Detect Pepper Plants disease', font='Arial 28 bold',
                             bg='#fff',fg='#b70000')
    ...
    def submit_fun(self):
        self.apple = pd.read_csv("text files/pepper_plant.csv", squeeze=True, usecols=["class"])
        self.catagory = list(self.apple)
        self.pickle_in = open("models/pepper_plant_Model.p", "rb")
        ...
        self.imgOriginal = cv2.resize(self.imgOriginal,(int(self.imgOriginal.shape[1])*2,int(self.imgOriginal.shape[0])*2))
        if self.probVal > 0.65:
            cv2.putText(self.imgOriginal, str(self.catagory[self.class_id]),(5,40),cv2.FONT_HERSHEY_SIMPLEX,1,(0,2,255),2)
            cv2.putText(self.imgOriginal, "acc = " + self.ac + '%', (5, 120), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255),2)
    ...
    def solution(self):
        ...
        self.sol = pd.read_csv("text files/pepper_plant.csv", squeeze=True, usecols=["sol"])
    ...
    def symptons(self):
        self.text.delete(1.0, END)
        self.sol = pd.read_csv("text files/pepper_plant.csv", squeeze=True, usecols=["lokkhon"])
        self.sol = list(self.sol)
        no_comma_str = self.sol[self.class_id].replace('#',',')
        string_t = ''
        for i in no_comma_str.split('@'):
    ...
    def solution(self):
        self.sol = pd.read_csv("text files/apple_fruit.csv", squeeze=True, usecols=["sol"])
        self.sol = list(self.sol)
        string_t = ''
        for i in self.sol[self.class_id].split('@'):
            string_t +=i+'\n'
        # print(string_t)
        self.text.insert(INSERT,string_t)
    ...
    def realtimetest(self):
        self.apple = pd.read_csv("text files/apple_fruit.csv", squeeze=True, usecols=["class"])
        self.catagory = list(self.apple)
