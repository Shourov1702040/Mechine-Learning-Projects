...
class Orange_fruit_frame(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry('800x650+1000+200')
        self.title("Detect Orange disease")
        ...
        img_jpg = Image.open(r"icon_image/orange.jpg")
        ...
        self.heading = Label(self.top, text='Detect Orange Fruit disease', font='Arial 28 bold',
                             bg='#fff',fg='#5860D5')
    ...
    ###################################   prediction function         ##################################
    def submit_fun(self):
        self.apple = pd.read_csv("text files/orange_fruit.csv", squeeze=True, usecols=["class"])
        self.catagory = list(self.apple)
        self.pickle_in = open("models/orange_fruit_Model.p", "rb")
    ...
    def solution(self):
        ...
        self.sol = pd.read_csv("text files/orange_fruit.csv", squeeze=True, usecols=["sol"])
    ...
    def symptons(self):
        self.text.delete(1.0, END)
        self.sol = pd.read_csv("text files/orange_fruit.csv", squeeze=True, usecols=["lokkhon"])
        self.sol = list(self.sol)
        no_comma_str = self.sol[self.class_id].replace('#',',')
        string_t = ''
        for i in no_comma_str.split('@'):
    ...
    def search_fun(self):
        ...
        try:
            ...
            self.text.insert(INSERT, "Cheak your internet connection !!!!!")
