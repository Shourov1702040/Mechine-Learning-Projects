...
class Cherry_plant_frame(Toplevel):
    def __init__(self):
        Toplevel.__init__(self)
        self.geometry('800x650+1000+200')
        self.title("Detect Cherry disease")
        ...
        img_jpg = Image.open(r"icon_image/cherry_plant.jpg")
        ...
        self.heading = Label(self.top, text='Detect Cherry Plants disease', font='Arial 28 bold',
                             bg='#fff',fg='#265909')
    ...
    def submit_fun(self):
        self.apple = pd.read_csv("text files/cherry_plant.csv", squeeze=True, usecols=["class"])
        self.catagory = list(self.apple)
        self.pickle_in = open("models/cherry_plant_Model.p", "rb")
    ...
    def solution(self):
        self.text.delete(1.0, END)
        self.sol = pd.read_csv("text files/cherry_plant.csv", squeeze=True, usecols=["sol"])
    ...
    def symptons(self):
        self.text.delete(1.0, END)
        self.sol = pd.read_csv("text files/cherry_plant.csv", squeeze=True, usecols=["lokkhon"])
        self.sol = list(self.sol)
        no_comma_str = self.sol[self.class_id].replace('#', ',')
        string_t = ''
        for i in no_comma_str.split('@'):
